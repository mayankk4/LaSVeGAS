...
def GetTopArticles(year, month, day, access='all-access', project='en.wikipedia.org'):
	url = WIKI_GET_TOP_ARTICLE_URL_API.replace("$$PROJECT", project).replace("$$ACCESS", access).replace("$$YEAR", year).replace("$$MONTH", month).replace("$$DAY", day)
	# print url
	titles = []
	data=""
	try:
...
def ExtractDetailedDescriptionByTitle(title, sentence_limit=3, search_limit=1):
	detailedTitlesInfo = []
	url = WIKI_GET_DESCRIPTION_BY_TITLE_API.replace("$$SENTENCE_LIMIT", str(sentence_limit)).replace("$$SERCH_LIMIT",str(search_limit)).replace("$$TITLE",title)
	print url
	response = urllib2.urlopen(url)
	data = json.loads(response.read())
	for k,v in data['query']['pages'].items():
		if 'extract' in v:
			...
			d['desc'] = v['extract'].encode('utf-8').replace("\n","").replace("\t","")
...
def FilterTitles(titles, allTitles, exclude_blacklist_titles):
	...
	if exclude_blacklist_titles:
		filteredTitles = [x for x in titles if x not in BLACKLIST_WIKI_ARTICLES_LIST]
...
def AllTitlesByRange(startDate, endDate, hop,exclude_blacklist_titles=True):
	...
	while start < stop:
		...
		filteredTitles = FilterTitles(titles, allTitles, exclude_blacklist_titles)
