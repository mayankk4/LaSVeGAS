...
imgFiles = [f for f in listdir(imgPath) if isfile(
    join(imgPath, f)) and f[-3:] == 'jpg']
...
for currentImg in iterImgs:
    try:
        # if the iterator will let us, grab the next file (and advance the
        # iterator pointer)
        nextImg = next(iterImgs)
        copyImageToOutput(nextImg)
        # use imagemagick to composite the two images and dump the result into
        # the output dir
        subprocess.call("composite -dissolve 50 -gravity South %s %s %s/%s_blended.jpg" %
                        (currentImg, nextImg, outputPath, currentImg), shell=True)
