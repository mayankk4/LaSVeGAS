...
def AvMux(state):
    ...
    for i in range(len(state.values)):
        value_image_path = state.path_to_output + "/value_image" + str(i) + ".png"
        value_audio_path = state.path_to_output + "/value_audio" + str(i) + ".mp3"
        value_output_path = state.path_to_output + "/value_video" + str(i) + ".mp4"
    ...
    cmd5 = "ffmpeg -y -f concat -i " + state.path_to_output + "/input_list.txt -c copy " + state.path_to_output + "/tmp.mp4"
    subprocess.call(cmd5, shell=True)

    output_path = state.path_to_output + "/final_output.mp4"
    cmd6 = "ffmpeg -y -i " + state.path_to_output + "/tmp.mp4 -codec:v libx264 -crf 21 -bf 2 -flags +cgop -pix_fmt yuv420p -codec:a aac " + \
